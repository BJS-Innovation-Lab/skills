...
async function main() {
  ...
  // ============== DATA QUALITY GATE ==============
  // Check if daily memory file exists and has real content
  const dailyDocs = docs.filter(d => d.file_path && d.file_path.includes(opts.date));
  const clientDocs = docs.filter(d => d.file_path && /clients\//.test(d.file_path) && d.file_path.includes(opts.date));
  const dailyContent = dailyDocs.reduce((s, d) => s + (d.content || '').length, 0);
  const dailyLines = dailyDocs.reduce((s, d) => s + (d.content || '').split('\n').filter(l => l.trim()).length, 0);
  
  // Total docs synced today (by created_at, regardless of filename)
  const totalSyncedToday = docs.length;
  
  // Data quality alert: agent was active (has synced docs) but daily notes are thin
  const DATA_QUALITY_THRESHOLD = 5; // minimum meaningful lines in daily file
  const isDailyThin = dailyLines < DATA_QUALITY_THRESHOLD;
  const hasNoClientNotes = clientDocs.length === 0;
  
  if (isDailyThin || docs.length === 0) {
    const alert = [
      `# ⚠️ DATA QUALITY ALERT: ${opts.agent}`,
      `**Date:** ${opts.date}`,
      ``,
      `## Problem`,
      docs.length === 0
        ? `❌ **No data at all** in Supabase for ${opts.agent} on ${opts.date}.`
        : `⚠️ **Daily memory file is nearly empty** (${dailyLines} lines, ${dailyContent} chars).`,
      ``,
      `| Metric | Value |`,
      `|--------|-------|`,
      `| Total docs synced today | ${totalSyncedToday} |`,
      `| Daily file lines | ${dailyLines} |`,
      `| Daily file chars | ${dailyContent} |`,
      `| Client interaction notes | ${clientDocs.length} |`,
      ``,
      `## What This Means`,
      `The agent ${totalSyncedToday > 0 ? 'synced infrastructure files but' : ''} **did not write meaningful daily notes**.`,
      `If the agent was active with clients today, those interactions are lost — not logged to memory, not available for reports.`,
      ``,
      `## Action Required`,
      `1. Check if ${opts.agent} had client sessions today (Telegram activity)`,
    ...
    fs.writeFileSync(path.join(reportDir, `${opts.agent}-${opts.date}-ALERT.md`), alert);
    console.error(`🚨 Data quality alert saved to memory/reports/${opts.agent}-${opts.date}-ALERT.md`);
    
    // Send alert to Santos via A2A
    if (opts.sendHQ) {
      sendViaA2A(alert, opts.date, 'DATA-QUALITY-ALERT');
    }
    
    // Exit with code 2 to distinguish from success (0) and error (1)
    process.exit(2);
  }
  ...
  if (analyses.length === 0) {
    console.log(`No substantive content for ${opts.agent} on ${opts.date}`);
    process.exit(0);
  }
