...
async function triggerOpenClawWake(message, attempt = 1) {
  ...
  // Try multiple paths for cross-platform support (macOS Homebrew, Linux, npm global)
  const openclawBin = process.env.OPENCLAW_BIN || 
    (require('fs').existsSync('/usr/local/bin/openclaw') ? '/usr/local/bin/openclaw' :
     require('fs').existsSync('/opt/homebrew/bin/openclaw') ? '/opt/homebrew/bin/openclaw' :
     require('fs').existsSync('/usr/bin/openclaw') ? '/usr/bin/openclaw' :
     'openclaw');
