...
### 3. Configure your agent (Auto-detected)

The script automatically detects your agent name from `IDENTITY.md` or your hostname.
Ensure your `IDENTITY.md` has a line like:
`**Name:** YourName`

If detection fails, you can force it when running:
`node sync-memory.cjs --agent yourname`

(Supported agents: sybil, sam, saber, santos, sage. If you are new, add your UUID to `AGENT_UUIDS` in the script.)
